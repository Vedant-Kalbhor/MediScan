...
MODEL_SAVE_NAME = Path(__file__).with_name("best_kidney_model.pth")
CLASS_NAMES_OUTPUT = Path(__file__).with_name("classes.json")
BENCHMARK_RESULTS_OUTPUT = Path(__file__).with_name("best_kidney_model_benchmark.csv")
METADATA_OUTPUT = Path(__file__).with_name("best_kidney_model_metadata.json")
